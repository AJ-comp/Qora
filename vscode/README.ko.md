...
Q#/C# 느낌의 문법으로 회로를 적고 **OpenQASM 3**로 변환합니다. 이 확장은 `.qor` 파일에 대해 문법
하이라이팅·설명·**실시간 오류 표시**·**변환 명령**을 제공합니다.
...
> `qora.command`(직접 만든 Qora 실행 파일) 또는 `qora.args`(+`dotnet`으로 `Qora.dll` 실행)로 연결할 수 있습니다.
