...
public sealed class HirLineage
{
    ...
    /// <summary>
    /// Returns whether <paramref name="ancestor"/> is the same generation as, or a structural ancestor
    /// of, <paramref name="from"/>. Both endpoints must belong to this exact Compilation snapshot.
    /// </summary>
    public bool IsAncestor(HirSnapshotId from, HirSnapshotId ancestor)
    {
        if (!_snapshots.ContainsKey(from))
            throw new ArgumentOutOfRangeException(
                nameof(from),
                from,
        ...
        for (HirSnapshotId? current = from;
             current is { } snapshot;
             current = _parents[snapshot])
        {
            if (snapshot == ancestor)
                return true;
        }
        return false;
    }
        ...
        HirNodeId nodeId)
    {
        ...
        {
            throw new ArgumentException(
                "HIR lineage cannot cross Compilation snapshots.",
                nameof(ancestor));
        }
        ...
        while (current.Snapshot != ancestor)
        {
            if (!visited.Add(current))
                throw new InvalidOperationException(
                    $"Cycle detected in HIR lineage at {current}.");

            if (_directOrigins.TryGetValue(current, out var direct))
            {
                current = direct;
                continue;
            }
            ...
            if (!parentSnapshot.Structure.Contains(current.NodeId))
            {
                throw new InvalidOperationException(
                    ...
                    "but no derivation connects it to the parent snapshot.");
            }
            ...
            if (currentKind != parentKind)
            {
                throw new InvalidOperationException(
                    ...
                    "across an implicit same-identity parent edge.");
            }
    ...
    /// <summary>
    /// Resolve source provenance rather than semantic identity. This query may cross a
    /// <see cref="NodeSynthesis"/> edge and therefore returns a full node reference whose ID and kind may
    /// differ from the synthesized node. Semantic-model consumers must use <see cref="ResolveNodeId"/>
    /// instead.
    /// </summary>
...
/// <summary>The cross-stage relationship data owned by one immutable Compilation snapshot.</summary>
public sealed class CrossStageLinks
{
    internal CrossStageLinks(
        HirCompilation hir,
        MirCrossStageLinks? mir)
    {
    ...
    public HirLineage Hir { get; }
    public MirCrossStageLinks? Mir { get; }
}

/// <summary>
/// A semantic view over a HIR generation derived from an analyzed ancestor. Lookups are translated through
/// <see cref="HirLineage"/> before reaching the exact semantic artifact; copied-node ancestry never lives in
/// the semantic model itself.
/// </summary>
internal sealed class HirSemanticContext : IHirSemanticContext
{
    internal HirSemanticContext(
        HirCompilation owner,
        HirSnapshot current,
        HirSemanticArtifact semanticBasis)
    {
        ArgumentNullException.ThrowIfNull(owner);
        Current = current ?? throw new ArgumentNullException(nameof(current));
        SemanticBasis = semanticBasis ?? throw new ArgumentNullException(nameof(semanticBasis));
        Lineage = owner.Lineage;
        SourceModel = semanticBasis.Model;

        if (!ReferenceEquals(owner.Find(current.Id), current))
        {
            throw new ArgumentException(
                $"Current HIR snapshot {current.Id} is detached from its owning HIR aggregate.",
                nameof(current));
        }
        if (!ReferenceEquals(
                owner.FindSemantics(
        ...
        {
            throw new ArgumentException(
                $"Semantic artifact {semanticBasis.Id} is detached from its owning HIR aggregate.",
                nameof(semanticBasis));
        }

        var canonicalValidation = owner.SpecializedValidation;
        var canonicalEffectAnalysis = owner.EffectAnalysis;
        ...
        {
            throw new ArgumentException(
                "A HIR lowering context requires the owning compilation's exact accepted canonical "
                + "effect-analysis artifact and its exact specialized-validation basis.",
                nameof(semanticBasis));
        }

        if (!Lineage.IsAncestor(current.Id, semanticBasis.SourceId))
            throw new ArgumentException(
                $"Semantic basis {semanticBasis.SourceId} is not an ancestor of current HIR {current.Id}.",
                nameof(semanticBasis));
    }

    public HirSnapshot Current { get; }
