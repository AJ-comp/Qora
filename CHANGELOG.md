...
### Changed
- **OpenQASM now lowers exclusively from the canonical SSA/CFG MIR snapshot.** The backend freezes a typed
  `MirOpenQasmTargetProgram` from that exact MIR revision and never reaches back into HIR or its semantic
  model. Diagnostics and emitted text therefore share one verified HIR-to-MIR-to-target provenance chain.
- **Compiler-generated inversion is now a MIR-only concept.** Exact MIR call sites carry internal inverse
  requests, and `MirAdjointMaterializer` creates the required inverse callables before target lowering.
  Qora source and HIR expose no adjoint or conjugation construct; automatic cleanup request injection
  remains a later compiler stage rather than a user-facing language feature.

### Fixed
- **Array-safety obligations now identify the exact indexed access.** Loads, stores, measurements, and
  individual indexed qubit operands retain typed MIR instruction, access-kind, and operand-position
  identities, including obligations copied into compiler-generated inverse callables.
- **Early returns inside `repeat` loops survive MIR-to-OpenQASM lowering.** Normal loop exits and callable
  returns are classified as distinct CFG edges, including nested loops and conditional returns.
- **Entry-point selection no longer mistakes a function named `Main` for an operation.** A runnable
  operation is selected consistently, while a program containing only functions reports QSEM040 instead
  of failing later as an internal MIR error.
- **Invalid internal inverse forms fail before emission.** Non-unitary instructions, unknown functors,
  duplicate functors, and non-canonical functor ordering are rejected by MIR verification.
- **MIR OpenQASM emission retains compiler provenance.** Emitted programs again begin with
  `// generated by Qora v0.32`, so generated artifacts identify the compiler version that produced them.

### Removed
- The legacy HIR inverter, conjugation and adjoint materialization passes, HIR uncompute planner/report,
  and the HIR-owned OpenQASM lowering and emitter stack. Their surviving responsibilities now have typed,
  revision-bound MIR or target-owned implementations.
...
- **Moved arguments can no longer be treated as reversible calls.** Ownership transfer is directional and
  cannot be restored by reversing statement order, so internal inverse validation rejects such a call and automatic cleanup
...
- **Previously-silent under-approximations in effect analysis are now loud invariant checks.** The branch
  ...
  than silently mis-mapping or dropping effects: a user-op call carrying an unsupported functor other
  than the compiler's internal inverse marker (QSEM002),
...
- **Documentation reorganized into two separated tracks, plus a new compiler datasheet.** The per-language
  pages now live under two trees — `docs/architecture/{lang}/` (the pipeline overview and H-gate intro)
  and `docs/book/{lang}/` (the 15-chapter learning book) — with
...
- **`within/apply` conjugation in the IR (`QConjugate` + `Ir/Passes/ConjugationLowering.cs`).** The
  ...
  `within` — installed *before* `AdjointMaterializer`, so every compiler-generated inverse marker becomes a real
...
- **Compiler-generated whole-operation inversion is now a real IR→IR pass.** A new `AdjointMaterializer`
  (`Ir/Passes/AdjointMaterializer.cs`) runs before name mangling: it turns each internal inverse marker on
  a user-operation call into an ordinary inverse-def op (`Foo__adj`, via the inversion kernel) and rewrites the
...
- New **compilation-pipeline architecture** page (en / ko / ja): source → parse → lower → the IR passes
  → emit, each stage with what it does and every listing produced by the real compiler.
...
- **Full-language execution path, measured and shipped.** Amazon Braket's LOCAL simulator runs
  ...
  (`tools/braket_validation.py`, now release gate #2). New `tools/run_braket.py` runs any `.qor` end-to-end
...
- **Hardened validation** (fourth adversarial review, 17 confirmed findings fixed):
  ...
  - full user-operation call signatures — argument count, register size, single-qubit vs register,
    and qubit-into-classical checks (QSEM006);
...
- **Semantic validation (QSEM001–QSEM015).** Invalid programs now fail *before* emission — with every
  violation reported in one compile — instead of silently producing broken OpenQASM: invalid
  functors on defs/reset (002/003), bare or in-expression measurement (004/005),
...
- **Grammar:** unary minus in expressions (`Rx(-pi/2, q[0]);`) and zero-argument gate/operation calls.
...
### Added
- **Controlled gates** use `Controlled G(...)`, emitted as OpenQASM `ctrl @`
