...
public class ArgumentTests
{
    ...
    // --- user operations ---
    [InlineData("operation Foo(Qubit[2] a){ H(a[0]); }\noperation Main(){ use q=Qubit[3]; Foo(q); }")]          // register size mismatch
    ...
    [InlineData("operation Foo(Qubit[n] q){ for i in 0..n-1 { X(q[i]); } }\noperation Main(){ use q=Qubit[2]; Foo(q[0]); }")] // generic register given a single qubit
    public void RejectsWrongArguments(string source) => Compiler.Rejects(source, "QSEM006");

    [Theory]
    // Regression: an INDEXED CLASSICAL in a value slot. Qora has no classical arrays, so `name[i]` is only
    // ever a qubit-reference form — never a scalar value. This once slipped past both value-slot checks and
    // silently emitted an invalid `rx(c[0])`; it must be QSEM006. (Found by adversarial review of CheckCall.)
    ...
    [InlineData("operation Foo(Qubit[2] a){ H(a[0]); }\noperation Main(){ use q=Qubit[2]; Foo(q); }")]           // exact register size
